...
export default function MoNewForm(props: Props) {
  ...
  const [usedSubLocations, setUsedSubLocations] = useState<string[]>([]);

  const subLocationOptions = Array.from(
    { length: 5 },
    (_, i) => `เขต ${selectedSector}.${i + 1}`,
  );

  // Filter out sublocations that already have a report today
  const availableSubLocationOptions = subLocationOptions.filter(
    (s) => !usedSubLocations.includes(s),
  );

  useEffect(() => {
    if (!authEmployee?.employee_code) return;
    fetchEmployeeTodayReportSubLocations(
      selectedSector,
      authEmployee.employee_code,
    ).then((reported) => {
      console.log("[MoNewForm] Already reported sublocations:", reported);
      setUsedSubLocations(reported);
      // Auto-select first available slot
      const available = subLocationOptions.filter((s) => !reported.includes(s));
      setSelectedSubLocation(available.length > 0 ? available[0] : "");
    });
  }, [selectedSector, authEmployee, fetchEmployeeTodayReportSubLocations]);
  ...
  const onSubmit = async (
    ...
    const payload: Record<string, unknown> = {
      ...
      sub_location: selectedSubLocation || availableSubLocationOptions[0] || "",
              ...
              <tbody>
                <tr>
                  ...
                  >
                    ...
                    >
                      {availableSubLocationOptions.map((sub, i) => (
                ...
                {openGroups[g.key] && (
                  <tbody>
                    ...
                    >
                      ...
                      <td colSpan={3} className={styles["add-row-cell"]}>
                        <div className={styles["add-row-centered"]}>
                          <PlusIcon className={styles["pin-icon"]} />
                          เพิ่มข้อมูลเข้าพบผู้ว่าจ้าง
                        </div>
