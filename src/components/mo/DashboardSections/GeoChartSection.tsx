...
const cityData: Record<string, GeoPoint[]> = Object.fromEntries(
  geoData.flatMap((c) =>
    (c.provinces ?? []).filter((p) => p.cities).map((p) => [p.id, p.cities!]),
  ),
);
...
const CAT_COLORS: {
  key: Exclude<keyof NonNullable<GeoPoint["cats"]>, "total">;
  label: string;
  color: string;
}[] = [
...
function CityLeafletMap({
  ...
  const boundary: Feature | null =
    cacheKey && boundaryCache.has(cacheKey)
  ...
  return (
    ...
    >
      ...
      {points.map((point) => {
        const t =
          maxVal === minVal ? 0.5 : (point.value - minVal) / (maxVal - minVal);
...
Object.entries(COUNTRY_PROJ).forEach(([k, v]) => {
  COUNTRY_CENTER[k] = v.center;
});
...
export default function GeoChart() {
  ...
  const rawPoints: GeoPoint[] = useMemo(() => {
    if (zoomLevel === "city" && selectedProvince)
      return cityData[selectedProvince] ?? [];
  ...
  const availableYears = useMemo(() => {
    ...
    rawPoints.forEach((p) => {
      if (p.createAt) yrs.add(Number(p.createAt.slice(0, 4)));
    });
  ...
  const availableMonths = useMemo(() => {
    ...
    rawPoints.forEach((p) => {
      ...
      if (
        selectedYear !== "all" &&
        Number(p.createAt.slice(0, 4)) !== selectedYear
      )
        return;
  ...
  const currentPoints: GeoPoint[] = useMemo(() => {
    return rawPoints.filter((p) => {
      ...
      if (
        selectedMonth !== "all" &&
        Number(p.createAt.slice(5, 7)) !== selectedMonth
      )
        return false;
      return true;
    });
  ...
  const mapZoom =
    zoomLevel === "world"
      ? 1
      : hasDedicatedMap || hasCountryProj
        ? 1
        : zoomLevel === "country"
          ? COUNTRY_ZOOM
          : 18;
  ...
  const geoUrl =
    zoomLevel === "world"
      ? WORLD_TOPO
      : selectedCountry && COUNTRY_TOPO[selectedCountry]
        ? COUNTRY_TOPO[selectedCountry]
        : COUNTRY_10M;

  // Whether to filter geographies to the selected country only (10m path)
  const filterToCountry =
    zoomLevel !== "world" && !hasDedicatedMap && !!selectedCountry;
  const selectedISO = selectedCountry
    ? (ISO_NUMERIC[selectedCountry] ?? null)
    : null;
  ...
  const selectedProvinceName = useMemo(() => {
    ...
    return (
      countryData[selectedCountry]?.find((p) => p.id === selectedProvince)
    ...
    <div className={styles.gcCard}>
      <div className="gc-header">
        ...
        <div className="gc-breadcrumb">
          ...
          {selectedProvince && (
            <>
              <span className="gc-sep">›</span>
              <span className="gc-crumb gc-crumb--active">
                {selectedProvinceName}
              </span>
            </>
          )}
          ...
          <CityLeafletMap
            ...
            fallbackCenter={(() => {
              if (!selectedCountry || !selectedProvince)
                return [13.75, 100.52] as [number, number];
              const prov = countryData[selectedCountry]?.find(
                (p) => p.id === selectedProvince,
              );
              return prov
                ? ([prov.lat, prov.lng] as [number, number])
            ...
            >
              ...
              {currentPoints.map((point) => {
                ...
                const canDrill =
                  (zoomLevel === "world" &&
                    !!point.country &&
                    !!COUNTRY_PROJ[point.country as string]) ||
                  zoomLevel === "country";
      ...
      {hasCats && selectedView === "categories" ? (
        <div className="gc-cat-legend">
          {CAT_COLORS.map((c) => (
            <div key={c.key} className="gc-cat-item">
              <span
