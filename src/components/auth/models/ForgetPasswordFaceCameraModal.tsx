...
import styles from "./ForgetPasswordFaceCameraModal.module.css";
import {
...
export default function ForgetPasswordFaceCameraModal({
  open,
  onClose,
  onCaptured,
  onReady,
  ...
  function setInvalidFaceMessage(message: string, immediate = false) {
    ...
    if (captureReadyRef.current || finalCaptureInFlightRef.current) {
      console.log("[ForgetPasswordFaceCameraModal] green cancelled message:", message);
    }
  ...
  function setValidFaceMessage(now: number, points: FeaturePoints) {
    ...
    if (!captureReadyRef.current) {
      ...
      resetAttributeCheck();
      resetAntiSpoofCheck();
      console.log("[ForgetPasswordFaceCameraModal] green ready baseline", {
    ...
    if (!autoCaptureTriggeredRef.current) {
      ...
      console.log("[ForgetPasswordFaceCameraModal] auto capture scheduled", {
        lockHoldMs,
      });
      autoCaptureTimerRef.current = window.setTimeout(() => {
        autoCaptureTimerRef.current = null;
        console.log("[ForgetPasswordFaceCameraModal] auto capture timer fired", {
      ...
      try {
        ...
        if (isGreenHold) {
          ...
          if (movement > faceDetectorSetting("max_capture_ready_face_movement")) {
            console.log("[ForgetPasswordFaceCameraModal] green hold cancelled by movement", {
              movement,
          ...
          if (lockMessage) {
            console.log("[ForgetPasswordFaceCameraModal] green hold cancelled by lock", {
              lockMessage,
  ...
  const capture = async (auto = false) => {
    ...
    const detector = detectorRef.current;
    console.log("[ForgetPasswordFaceCameraModal] capture requested", {
      auto,
      hasVideo: !!video,
    ...
    if (!video || !previewCanvas || !canvas || !detector || (!auto && !captureReady)) {
      console.log("[ForgetPasswordFaceCameraModal] capture skipped: missing dependency or not ready");
      return;
    }
    if (finalCaptureInFlightRef.current) {
      console.log("[ForgetPasswordFaceCameraModal] capture skipped: already in flight");
      return;
    }

    const failCapture = (message: string) => {
      console.log("[ForgetPasswordFaceCameraModal] capture failed", { message });
      finalCaptureInFlightRef.current = false;
    ...
    setBusy(true);
    console.log("[ForgetPasswordFaceCameraModal] final capture started");
    ...
    const face = faces[0];
    console.log("[ForgetPasswordFaceCameraModal] final face detected", {
      faces: faces.length,
    ...
    console.log("[ForgetPasswordFaceCameraModal] capture success", {
      width,
      height,
